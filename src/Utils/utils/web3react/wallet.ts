...
export const setupNetwork = async (chain: string) => {
  ...
  if (provider) {
    const chainConfig = CHAIN_CONFIGS[process.env.ENV][chain];
    if (!chainConfig) return false;
    try {
      console.log("starting chain swithing");
      ...
      console.log("endign chain swithing");
...
export const switchNetwork = async (chain: string) => {
  ...
  if (provider) {
    const chainConfig = CHAIN_CONFIGS[process.env.ENV][chain];
    console.log(" chainConfig.chainIdHex : ", chainConfig.chainIdHex);
